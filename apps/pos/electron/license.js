 ...
 *     1. Run `npm run setup` inside apps/license-server/ to generate your Ed25519 key pair.
      ...
      res.on('end', () => {
        try {
          ...
          if (parsed.success && parsed.token) {
            ...
            resolve({
              success: true,
              // Only present on a genuine first-time activation whose
              // license had preset credentials — the server serves these
              // exactly once and clears them, so a later re-activation
              // never receives them again.
              presetAdminEmail: parsed.preset_admin_email || null,
              presetAdminPassword: parsed.preset_admin_password || null,
            });
          } else {

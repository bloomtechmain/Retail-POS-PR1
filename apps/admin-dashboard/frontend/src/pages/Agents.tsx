...
export default function Agents() {
  ...
  return (
    <div>
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-surface-900">System Users</h1>
        <p className="text-surface-500 text-sm mt-1">
          {isAdmin ? 'Everyone with access to this dashboard.' : 'Everyone with access to this dashboard — only admins can add or disable agents.'}
        </p>
      </div>
        ...
        <table className="table">
          ...
          <tbody>
            ...
            {!loading && agents.length === 0 && (
              <tr><td colSpan={6} className="text-center text-surface-400 py-8">No users yet.</td></tr>
            )}
            {agents.map((a) => (
              <tr key={a.id}>
